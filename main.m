clear, clc;
close all;

%% User input section of the code. Made by Fielding McKnight
disp('Enter the heights of the five roller coaster peaks.');

yPeaks = zeros(1,5);

% Repeatedly ask the user for input on the heights of 5 peaks on the roller
% coaster.
for i = 1:5
    yPeaks(i) = input(['Enter height for peak ', num2str(i), ': ']);
    
    % If the user enters a peak height that is greater than the first peak
    % height, the roller coaster will stall out, and will be unable to
    % complete the track. Because of this , we reject any invalid input and
    % ask the user for a valid, smaller number. Additionally, a peak height
    % smaller than zero will result in a bug in our rail trajectory code,
    % so we also reject input that is below a height of 0.
    while yPeaks(i) <= 0 || (i > 1 && yPeaks(i) > yPeaks(1))
        disp('Invalid input. Height must be greater than 0 and not exceed the first peak.');
        yPeaks(i) = input(['Enter height for peak ', num2str(i), ': ']);
    end
end

%% Final and initial kinetic and potential energy calculation. Created by Boen Kelly

%Caluclate the ending height so that the final velocity is 9 m/s
m = 10;   % kg
V = 9;   % m/s
g = 9.81; % m/s^2
endingH = (g*yPeaks(1) - .5*(V)^2)/g; %m

finalKE = .5*m*(V)^2;
finalPE = m*g*yPeaks(1);

%% Roller coaster path generation code. Created by Trajan Falk

% Start by getting five points from user input, each representing a 
% peak/starting point. Set the final height to the highest peak minus four
% so that the final velocity is less than 10 m/s 
y0(1)=yPeaks(1);
y0(3)=yPeaks(2);
y0(5)=yPeaks(3);
y0(7)=yPeaks(4);
y0(9)=yPeaks(5);
y0(11)=endingH;

% Then, generate x coordinates. The peaks will be located every 60 meters
% (to make sure the coaster is at least 300 m). In between each peak, we
% have a random x-coordinate that is between 20 & 40 m from each peak.
xPeaks=[0,randi([20,40]),60,randi([80,100]),120,randi([140,160]),180,randi([200,220]),240,randi([260,280]),300];

% Now, we generate troughs. The height of the troughs is located at a
% random location that is below the lower of the two peaks that it sits
% between.
y0(2)=randi([0,min([y0(1),y0(3)])]);
y0(4)=randi([0,min([y0(3),y0(5)])]);
y0(6)=randi([0,min([y0(5),y0(7)])]);
y0(8)=randi([0,min([y0(7),y0(9)])]);
y0(10)=randi([0,min([y0(9),y0(11)])]);

% Now, we use a assume that the rollercoaster path p(x) is modeled by a
% polynomial. As I will describe, we have 6 conditions that need to be met,
% so we will use a 5th order polynomial. For three conditions, the
% polynomial must equal the values of y at the values of x. For the other
% three, the derivative of y with respect to x at these three points must
% be zero. Through my calculations, I have determined that the polynomial 
% will take the form:
% y(x) = int_0^x((x-xPeaks(1))(x-xPeaks(2))(x-xPeaks(3))(Ax+b))dx+y0(1)
%      = (A)/5.*x.^5+(b-A.*(xPeaks(1)+xPeaks(2)+xPeaks(3))/4.*x.^4+(A*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-b*(xPeaks(1)+xPeaks(2)+xPeaks(3)))/3.*x.^3+(b*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-A*xPeaks(1)*xPeaks(2)*xPeaks(3))/2.*x.^2-b*xPeaks(1)*xPeaks(2)*xPeaks(3).*x+y0(1)
% Also through my own calculations, I have found a pair of equations that
% can be used to solve for A and B: They are as follows
% Equation 1:
% ((A)/5.*xPeaks(2).^5+(b-A.*(xPeaks(1)+xPeaks(2)+xPeaks(3))/4.*xPeaks(2).^4+(A*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-b*(xPeaks(1)+xPeaks(2)+xPeaks(3)))/3.*xPeaks(2).^3+(b*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-A*xPeaks(1)*xPeaks(2)*xPeaks(3))/2.*xPeaks(2).^2-b*xPeaks(1)*xPeaks(2)*xPeaks(3).*xPeaks(2))-((A)/5.*xPeaks(1).^5+(b-A.*(xPeaks(1)+xPeaks(2)+xPeaks(3))/4.*xPeaks(1).^4+(A*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-b)/3.*xPeaks(1).^3+(b*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-A*xPeaks(1)*xPeaks(2)*xPeaks(3))/2.*xPeaks(1).^2-b*xPeaks(1)*xPeaks(2)*xPeaks(3).*xPeaks(1))+y0(1)-y0(2)=0
% Equation 2:
% ((A)/5.*xPeaks(3).^5+(b-A.*(xPeaks(1)+xPeaks(2)+xPeaks(3))/4.*xPeaks(3).^4+(A*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-b*(xPeaks(1)+xPeaks(2)+xPeaks(3)))/3.*xPeaks(3).^3+(b*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-A*xPeaks(1)*xPeaks(2)*xPeaks(3))/2.*xPeaks(3).^2-b*xPeaks(1)*xPeaks(2)*xPeaks(3).*xPeaks(3))-((A)/5.*xPeaks(2).^5+(b-A.*(xPeaks(1)+xPeaks(2)+xPeaks(3))/4.*xPeaks(2).^4+(A*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-b)/3.*xPeaks(2).^3+(b*(xPeaks(1)*xPeaks(2)+xPeaks(2)*xPeaks(3)+xPeaks(3)*xPeaks(1))-A*xPeaks(1)*xPeaks(2)*xPeaks(3))/2.*xPeaks(2).^2-b*xPeaks(1)*xPeaks(2)*xPeaks(3).*xPeaks(2))+y0(2)-y0(3)=0
% This process must be carried out between each peak. We will make a for
% loop to do this.

for j=1:2:9

    syms A b;
    
    % Helpful intermediate sums to keep the equations readable
    S1 = xPeaks(j) + xPeaks(j+1) + xPeaks(j+2);
    S2 = xPeaks(j)*xPeaks(j+1) + xPeaks(j+1)*xPeaks(j+2) + xPeaks(j+2)*xPeaks(j);
    S3 = xPeaks(j)*xPeaks(j+1)*xPeaks(j+2);
    
    % Define the indefinite integral WITHOUT the constant of integration
    F = @(x, A_sym, b_sym) (A_sym/5).*x.^5 + (b_sym-A_sym*S1)/4.*x.^4 + (A_sym*S2-b_sym*S1)/3.*x.^3 + (b_sym*S2-A_sym*S3)/2.*x.^2 - b_sym*S3.*x;
    
    % The true expression shifts the curve so it evaluates exactly to y0(j) when x = xPeaks(j)
    expr = @(x, A_sym, b_sym) F(x, A_sym, b_sym) - F(xPeaks(j), A_sym, b_sym) + y0(j);
    
    % Set up the equations: y(xPeaks(j+1)) = y0(j+1) & y(xPeaks(j+2)) = y0(j+2)
    eq1 = expr(xPeaks(j+1),A,b) == y0(j+1);
    eq2 = expr(xPeaks(j+2),A,b) == y0(j+2);
    
    % Now, we need to solve the system of equations for A & b
    sol = solve([eq1, eq2], [A, b]);
    A_val = double(sol.A);
    b_val = double(sol.b);
    
    % Calculate the correct constant term (the y-intercept) for the polynomial array
    C_val = y0(j) - F(xPeaks(j), A_val, b_val);
    
    % Create the polynomial vector using the clean S1, S2, S3 variables
    p = [A_val/5, (b_val-A_val*S1)/4, (A_val*S2-b_val*S1)/3, (b_val*S2-A_val*S3)/2, -b_val*S3, C_val];
    
    % Create space and evaluate
    x_space((50*j-49):(50*j+50)) = linspace(xPeaks(j),xPeaks(j+2),100);
    y((50*j-49):(50*j+50))=polyval(p,x_space((50*j-49):(50*j+50)));

end

%% Animation code. Created by Trajan Falk

% --- 1. CONTINUOUS KINETIC ENERGY & SPEED CALCULATION ---
% (This turns v into an array of 500 speeds so the animation works)
m=100; g=9.81;
U = m * g * y;
T = max(U) - U;
v = sqrt(2*T/m);

% --- 2. PLOT THE TRACK ---
figure;
plot(xPeaks,y0,'*',x_space,y)
grid on;
xlabel('Horizontal Distance (m)');
ylabel('Height (m)');
legend('Peaks & Troughs', 'Location', 'best');

% --- 3. FIXED FPS ANIMATION ---
hold on;
cart = plot(x_space(1), y(1), 'r.', 'MarkerSize', 40);

% Force row vectors just to be absolutely safe against dimension mismatches
x_space = x_space(:)';
y = y(:)';
v = v(:)';

% Calculate arc length and average velocity
dx = diff(x_space);
dy = diff(y);
ds = sqrt(dx.^2 + dy.^2);

v_avg = (v(1:end-1) + v(2:end)) / 2;
v_avg(v_avg == 0) = 0.01; % Prevent division by zero

% Calculate time intervals and cumulative time array
dt = ds ./ v_avg;
t_cum = [0, cumsum(dt)]; 

% Create a fixed time array for a smooth 30 FPS animation
fps = 30;
dt_frame = 1 / fps;
t_anim = 0 : dt_frame : t_cum(end); 

% Interpolate X, Y, and Velocity at these exact time intervals
[t_unique, idx] = unique(t_cum);
x_anim = interp1(t_unique, x_space(idx), t_anim, 'pchip');
y_anim = interp1(t_unique, y(idx), t_anim, 'pchip');
v_anim = interp1(t_unique, v(idx), t_anim, 'pchip');

title_obj = title(sprintf('Roller Coaster Speed: %.1f m/s', v_anim(1)));
disp('Starting fixed-timestep animation...');

% Display initial PE and final PE and KE
energyDisp = sprintf("MAX PE = %fJ\nFINAL KE = %fJ\nFinal Velocity = %f m/s",g*yPeaks(1),finalKE, v_anim(length(t_anim)));

text(150,y0(1)-10,energyDisp)

% Run the animation loop
for i = 1:length(t_anim)
    
    set(cart, 'XData', x_anim(i), 'YData', y_anim(i));
    set(title_obj, 'String', sprintf('Roller Coaster Speed: %.1f m/s', v_anim(i)));
    
    drawnow; 
    pause(dt_frame); 
end

disp('Animation complete!');